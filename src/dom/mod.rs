...
// Document, Document Fragment, or Empty
...
#[serde(rename_all = "camelCase")]
pub struct Dom {
    // Type of the tree that was parsed
...
impl Dom {
    ...
    fn build_dom(pairs: Pairs<'_, Rule>) -> Result<Self> {
            ...
            1 => match dom.children[0] {
                Node::Element(ref el) => {
                    ...
                    if name == "html" {
                        ...
                        Err(
                            Error::Parsing("A document can only have html as root".to_string()).into(),
                            )
                    } else {
                        dom.tree_type = DomVariant::DocumentFragment;
                        Ok(dom)
                    ...
                    if let Node::Element(ref el) = node {
                        ...
                        if name == "html" || name == "body" || name == "head" {
                            return Err(Error::Parsing(format!(
                                        "A document fragment should not include {}", name
                                        ))
                                .into());
                        }
                    }
    ...
    fn build_node_element(pairs: Pairs<'_, Rule>) -> Result<Option<Node>> {
    ...
    fn build_attribute(pairs: Pairs<'_, Rule>) -> Result<(String, Option<String>)> {
